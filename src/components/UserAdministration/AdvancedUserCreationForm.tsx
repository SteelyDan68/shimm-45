...
interface AdvancedUserCreationFormProps {
  onUserCreated?: () => void;
  onCancel?: () => void;
}
...
export function AdvancedUserCreationForm({ onUserCreated, onCancel }: AdvancedUserCreationFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userData = {
        ...
        extendedProfile: {
          personnummer: formData.personnummer.replace(/\D/g, ''),
      ...
      const { data, error } = await supabase.functions.invoke('admin-create-user-enhanced', {
        body: userData
      });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Skapa ny användare</h2>
          <p className="text-muted-foreground">Komplett användarregistrering med CRM-funktionalitet</p>
        </div>
      </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                12 siffror i format YYYYMMDD-XXXX
              </p>
            </div>
