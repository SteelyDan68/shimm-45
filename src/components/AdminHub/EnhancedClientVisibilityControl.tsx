...
export function EnhancedClientVisibilityControl() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* STEFAN HALLGREN SPECIAL HIGHLIGHT */}
      {clients.find(c => c.email === 'stefan.hallgren@happyminds.com') && (
        <Card className="border-green-200 bg-green-50/30">
          <CardHeader>
            <CardTitle className="text-green-700 flex items-center gap-2">
              ✅ Stefan Hallgren Status
          ...
          <CardContent>
            {(() => {
              const stefan = clients.find(c => c.email === 'stefan.hallgren@happyminds.com');
              return stefan ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="text-center p-3 bg-white rounded-lg">
                    <div className="text-lg font-bold text-green-600">{stefan.assigned_coaches.filter(c => c.is_active).length}</div>
                    <div className="text-sm text-muted-foreground">Aktiva coaches</div>
                  </div>
                  <div className="text-center p-3 bg-white rounded-lg">
                    <div className="text-lg font-bold text-blue-600">{stefan.pending_tasks}</div>
                    <div className="text-sm text-muted-foreground">Uppgifter i kalender</div>
                  </div>
                  <div className="text-center p-3 bg-white rounded-lg">
                    <div className="text-lg font-bold text-purple-600">{stefan.upcoming_events}</div>
                    <div className="text-sm text-muted-foreground">Kommande events</div>
                  </div>
                </div>
              ) : null;
            })()}
          ...
          {filteredClients.map(client => (
            <Card 
              ...
              className={`hover:shadow-lg transition-all cursor-pointer ${
                ...
                client.email === 'stefan.hallgren@happyminds.com' ? 'border-green-300 bg-green-50/50' : ''
              }`}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {client.email === 'stefan.hallgren@happyminds.com' && (
                        <Badge variant="outline" className="text-xs bg-green-100 text-green-800 ml-1">
                          ⭐ TEST KLIENT
                        </Badge>
                      )}
                    </div>
