...
export const ModernMessageBubble: React.FC<ModernMessageBubbleProps> = ({
  ...
  return (
    <div className={cn(
      ...
      {/* Sender name - ALWAYS show for better identification */}
      <div className={cn(
        "text-xs font-medium px-2",
        isStefanAI ? "text-green-600 font-semibold" : isOwn ? "text-blue-600 font-semibold" : "text-muted-foreground"
      )}>
        {isStefanAI ? "🤖 Stefan AI" : isOwn ? (message.sender_profile?.first_name || "Du") : message.sender_profile?.first_name || "Okänd"}
      </div>
      ...
      <div className={cn(
        ...
        isStefanAI ? (
          ...
          // Own messages (right side) - FORCED BLUE COLOR for client
        ...
        {showTimestamp && (
          <div className={cn(
            ...
            isStefanAI ? "text-white/70" : isOwn ? "text-white/70" : "text-muted-foreground"
          )}>
