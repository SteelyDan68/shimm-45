...
BEGIN
  ...
  -- 2. Delete development strategies for this pillar
  ...
  WHERE user_id = p_user_id 
    AND assessment_round_id IN (
      SELECT ar.id FROM assessment_rounds ar 
      WHERE ar.user_id = p_user_id AND ar.pillar_type = p_pillar_type
    );
  GET DIAGNOSTICS deleted_mappings = ROW_COUNT;

  -- 7. Delete coaching milestones related to this pillar's plans
  ...
  -- 8. Update coaching plans to remove this pillar from focus areas
