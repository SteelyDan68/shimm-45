-- =====================================================
-- 🔄 RETAKE SYSTEM REDESIGN - FULL SYSTEM INTEGRITY
-- =====================================================
...
-- Clean up Anna's duplicate self_care assessments immediately
DO $$
DECLARE
  cleanup_result RECORD;
BEGIN
  RAISE LOG 'Cleaning up Annas duplicate self_care assessments...';
  
  -- First, let's see what we have
  RAISE LOG 'Current self_care assessments for Anna: %', 
    (SELECT array_agg(row_to_json(ar.*)) 
     FROM assessment_rounds ar
     JOIN profiles p ON p.id = ar.user_id 
     WHERE p.email LIKE '%anna%' AND ar.pillar_type = 'self_care');
  ...
  WHERE id IN (
    ...
    FROM assessment_rounds ar
    JOIN profiles p ON p.id = ar.user_id
    WHERE p.email LIKE '%anna%' 
      AND ar.pillar_type = 'self_care'
      AND ar.created_at < (
        SELECT MAX(created_at) 
        FROM assessment_rounds ar2 
        JOIN profiles p2 ON p2.id = ar2.user_id
        WHERE p2.email LIKE '%anna%' 
          AND ar2.pillar_type = 'self_care'
      )
  );
  
  RAISE LOG 'Anna self_care cleanup completed';
END;
$$;