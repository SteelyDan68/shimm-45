...
-- 1. Fix user_presence table - Remove overly permissive policy
...
-- 5. Create trigger for profile access logging
...
AS $$
BEGIN
  -- Only log when non-owner accesses profile
  IF auth.uid() != NEW.id AND auth.uid() != OLD.id THEN
...
-- 6. Enhance invitation security with better validation
