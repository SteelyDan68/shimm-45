...
CREATE POLICY "Coaches can view client intelligence profiles" 
...
USING (
  ...
  OR
  -- Allow coaches to see profiles of users they have relationships with
  EXISTS (
    SELECT 1 FROM public.user_relationships 
    WHERE coach_id = auth.uid() 
    AND client_id = intelligence_profiles.client_id
  )
  OR
