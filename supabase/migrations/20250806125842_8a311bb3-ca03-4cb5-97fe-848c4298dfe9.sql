-- Fix delete_user_completely function to handle soft-deleted users and missing columns gracefully
...
BEGIN
  -- Try to find user in profiles table (including inactive users)
  SELECT email INTO user_email FROM profiles WHERE id = user_uuid;
  
  IF user_email IS NULL THEN
    -- Try to find by email parameter if user_uuid is actually an email
    SELECT email, id INTO user_email, user_uuid FROM profiles WHERE email = user_uuid::text;
  ...
  -- 3. Coach client assignments - check if column exists
  ...
  -- 10. Finally delete the profile (whether active or inactive)
