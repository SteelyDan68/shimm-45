-- ============================================
-- STEFAN AI COMPREHENSIVE DATABASE STRUCTURE (FIXED)
...
CREATE POLICY "stefan_memory_system_access" 
ON public.stefan_memory FOR ALL 
WITH CHECK (true);

-- Context Events Policies  
CREATE POLICY "context_events_user_select" 
ON public.user_context_events FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY "context_events_system_insert" 
ON public.user_context_events FOR INSERT 
WITH CHECK (true);

-- Behavior Patterns Policies
CREATE POLICY "behavior_patterns_user_select" 
ON public.user_behavior_patterns FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY "behavior_patterns_system_all" 
ON public.user_behavior_patterns FOR ALL 
WITH CHECK (true);

-- Proactive Interventions Policies
CREATE POLICY "interventions_user_select" 
...
CREATE POLICY "interventions_system_all" 
ON public.proactive_interventions FOR ALL 
WITH CHECK (true);

-- Stefan Config Policies (Admin only)
CREATE POLICY "stefan_config_admin_all" 
ON public.stefan_ai_config FOR ALL 
USING (is_admin(auth.uid()));

CREATE POLICY "stefan_config_public_select" 
ON public.stefan_ai_config FOR SELECT 
USING (is_active = true);

-- Analytics Policies
CREATE POLICY "stefan_analytics_user_select" 
...
CREATE POLICY "stefan_analytics_admin_select" 
ON public.stefan_analytics FOR SELECT 
USING (is_admin(auth.uid()));

-- AI Service Logs Policies
CREATE POLICY "ai_logs_system_all" 
...
CREATE POLICY "neuroplasticity_user_select" 
ON public.neuroplasticity_progress FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY "neuroplasticity_user_all" 
ON public.neuroplasticity_progress FOR ALL 
USING (auth.uid() = user_id);

CREATE POLICY "neuroplasticity_system_all" 
ON public.neuroplasticity_progress FOR ALL 
WITH CHECK (true);
