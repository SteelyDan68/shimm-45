-- 🚀 PERFORMANCE OPTIMIZATION - RENA DATABASE INDEXES
-- Phase 2: Ingen tidsbaserade predikater (immutable constraint)
...
-- 5. AI_CONVERSATIONS OPTIMIZATION (conditional check)
DO $$ 
BEGIN
...
-- 6. PROFILE_METADATA OPTIMIZATION (conditional check)
DO $$ 
BEGIN
...
-- 7. PATH_ENTRIES OPTIMIZATION (conditional check)
DO $$ 
BEGIN
...
-- 8. TASKS OPTIMIZATION (conditional check)
DO $$ 
BEGIN
...
-- 9. ORGANIZATION_MEMBERS OPTIMIZATION (conditional check)
DO $$ 
BEGIN
...
-- Only analyze tables that definitely exist
