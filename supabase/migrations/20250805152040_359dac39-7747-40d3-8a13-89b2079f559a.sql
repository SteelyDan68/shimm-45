...
CREATE OR REPLACE FUNCTION public.get_user_id_from_client_id(client_uuid uuid)
 RETURNS uuid
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT user_id FROM public.clients WHERE id = client_uuid;
$function$;

CREATE OR REPLACE FUNCTION public.get_client_id_from_user_id(user_uuid uuid)
 RETURNS uuid
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
